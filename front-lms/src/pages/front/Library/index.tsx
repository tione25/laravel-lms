...
const Library = () => {
  ...
  const {
    ...
    queryFn: async ({ pageParam }) => {
      const res = await client.get(
        `/library?category=${searchParams.get("category") ?? ""}&&order=${
  ...
  return (
    <>
      <Helmet>
        <title>Library | CourseCasts</title>
      </Helmet>

      <Flex flexDir="column">
        {/* Simple Page Hero */}
        <SimpleHero title="Library" />
        ...
        >
          <Box fontSize="md" display="flex" gap={2} color="gray.400">
            We found <Text color="gray.600">{data?.pages.flatMap((page) => page.data)?.length ?? 0} courses</Text> available for you
          </Box>
          <Flex
            w={{ base: "full", md: "auto" }}
            gap={4}
                ...
                >
                  {group.data.map((course) => (
                    <CourseCard
                      course={course}
                      key={course.id}
                      percentage={course.enrollments?.[0]?.percentage_completed}
                    />
                  ))}
                </SimpleGrid>
              </Box>
